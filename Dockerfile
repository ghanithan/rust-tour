...
RUN cargo chef cook --profile ci-release --recipe-path recipe.json --no-default-features
...
RUN cargo build --profile ci-release --package rust-tour --no-default-features
...
COPY --from=rust-builder /app/target/ci-release/rust-tour /usr/local/bin/rust-tour
...
# Copy web assets (for non-embedded builds)
COPY --from=rust-builder --chown=rustuser:rustuser /app/web/dist ./web/dist
